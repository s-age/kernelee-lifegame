...
/**
 * The 4 entries that scanCommandEndpoints promotes to command endpoints, which
 ...
 * have different counts". play/pause/step/strokeEnd are bound portK members
 * with no corresponding describePipe (the Mutators in running.mutator.ts / the
 * one-line launcher in stepOnce.ts).
 */
const COMMAND_PROMOTED_UNLISTED: readonly WiringIssueAllowlistEntry[] = [
  { kind: 'unlistedBoundSymbol', key: 'Circuit.Sim.play' },
